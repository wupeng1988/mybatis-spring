...
import org.apache.ibatis.builder.xml.SingletonJpaMapperBuilder;
...
public class SqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>, InitializingBean, ApplicationListener<ApplicationEvent> {
    ...
    /**
     * set mapper packages, support for jpa features
    ...
    protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
        ...
        if (!isEmpty(this.mapperPackages)) {
            for (String mapperPackage : this.mapperPackages) {
                if (StringUtils.isEmpty(mapperPackage)) {
                    continue;
                }

                List<Class> classes = ClassScanner.scan(mapperPackage);
                for (Class clazz : classes) {
                    if (Mapper.class.isAssignableFrom(clazz)) {
                        try {
                            ...
                            throw new NestedIOException("Failed to parse jpa mapping class: '" + clazz + "'", e);
                        } finally {
                            ErrorContext.instance().reset();
                        }

                        if (LOGGER.isDebugEnabled()) {
                            LOGGER.debug("Parsed mapper file: '" + clazz + "'");
                        }
                    } else {
                        LOGGER.error("class " + clazz + " not instance of Mapper");
                    }
                }
            }
        } else {
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("Property 'mapperPackages' was not specified or no matching resources found");
            }
        }
